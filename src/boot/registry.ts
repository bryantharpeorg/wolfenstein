 ...
 *
 * WHY THIS EXISTS. Every story of 001 edited `src/main.ts` — us1, us2 and us3 all
 * touched it. Two stories that are logically independent still collide there, and the
 * merge queue rejects whichever lands second for a conflict neither caused. Ergane
 * prices that rejection as a code defect and it costs the node a ladder rung, so a
 * shared wiring file converts parallelism directly into rework.
 *
 * A story now adds `src/systems/<name>/register.ts` and nothing else. Discovery is by
 * glob (see `discover.ts`), so there is no index to append to either — an index file
 * conflicts on adjacent lines just as readily as a wiring function does.
 */
