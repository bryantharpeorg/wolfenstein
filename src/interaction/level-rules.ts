...
// (FR-011's key placement, FR-014's secret placement). An index would put their
// one-line additions on adjacent lines of one file, which is a conflict; a glob
// makes a new rule a new file and nothing else. Pure: no DOM, no three.js, and
// vitest runs it because vitest transforms through Vite.
 ...
 * file declares its own category by augmenting this from its own module — the
 * same trick `interaction-diag.ts` uses on 001's `Diagnostics` — so no rule ever
 * has to widen a union declared in someone else's file.
 */
...
function isRuleModule(value: unknown): value is LevelRuleModule {
  return typeof value === 'object' && value != null && typeof (value as LevelRuleModule).rule === 'function';
}
