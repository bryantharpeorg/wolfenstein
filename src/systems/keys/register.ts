...
function publishKeys(): void {
  ...
  // Unchanged by every unlock, and reported as such rather than assumed (FR-010).
...
defineSystem({
  ...
  setup(ctx) {
    ...
    // The lock reads the live inventory, so a key collected mid-session counts on
    // the next press without re-registration (FR-009).
    registerDoorGate(
  ...
  update() {
    ...
    if (refusedKey != null) {
      ...
      // wanted, so the HUD and the harness read a refusal, not a dead press.
