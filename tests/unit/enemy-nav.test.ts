...
/** A 64x64 room split at x=32, with the only gap a door at (32, 10). */
...
describe('the navigator throttles path requests per guard (FR-004, Edge Cases)', () => {
  it('issues at most one search per guard per declared interval', () => {
    const nav = createNavigator({ grid: OPEN_ROOM, doorStates: () => new Set() });
    ...
    for (let tick = 1; tick < PATH_REQUEST_INTERVAL_TICKS; tick += 1) {
      nav.beginTick(tick);
      const repeat = world.findPath(cell(2, 2), goal);
  ...
  it('reports the throttle interval it applied, so a regression is visible', () => {
    const nav = createNavigator({ grid: OPEN_ROOM, doorStates: () => new Set() });
    ...
    expect(nav.report().throttled).toBe(0);
  });
  ...
  it('never lets a single search exceed the declared expansion cap', () => {
    const nav = createNavigator({ grid: LEVEL_GRID, doorStates: () => new Set() });
    nav.beginTick(0);
...
describe('the navigator discards a stale path (Edge Cases)', () => {
  it('re-searches inside the throttle window when the goal cell changes', () => {
    const nav = createNavigator({ grid: OPEN_ROOM, doorStates: () => new Set() });
    const world = nav.worldFor('alpha');
  ...
  it('does not count an unchanged goal as stale', () => {
    const nav = createNavigator({ grid: OPEN_ROOM, doorStates: () => new Set() });
    const world = nav.worldFor('alpha');
    nav.beginTick(0);
    world.findPath(cell(2, 2), cell(20, 20));
...
describe('a door closing mid-traverse forces a new path (Edge Cases)', () => {
  it('re-searches within one tick and drops the route through the closed door', () => {
...
describe('two guards converging on one corridor cell (FR-003, Edge Cases)', () => {
  it('gives each guard a distinct claimed cell, and neither one a wall', () => {
    const nav = createNavigator({ grid: CORRIDOR, doorStates: () => new Set() });
    ...
    const alphaClaim = nav.claimedCell('alpha');
    const betaClaim = nav.claimedCell('beta');
    expect(alphaClaim).not.toBeNull();
    expect(betaClaim).not.toBeNull();
    expect(alphaClaim).not.toEqual(betaClaim);

    for (const claim of [alphaClaim, betaClaim]) {
      expect(isTileBlocking(CORRIDOR, claim!.x, claim!.z, new Set())).toBe(false);
    }
    // Neither stops moving: both still hold a path with somewhere to go.
    expect(cellsOf(alpha).length).toBeGreaterThan(1);
    expect(cellsOf(beta).length).toBeGreaterThan(1);
    expect(cellsOf(alpha)[cellsOf(alpha).length - 1]).toEqual(alphaClaim);
    expect(cellsOf(beta)[cellsOf(beta).length - 1]).toEqual(betaClaim);
  });

  it('keeps the claim within the declared search radius of the cell asked for', () => {
    const nav = createNavigator({ grid: CORRIDOR, doorStates: () => new Set() });
    nav.beginTick(0);
    nav.worldFor('alpha').findPath(cell(1, 5), cell(10, 5));
    nav.worldFor('beta').findPath(cell(20, 5), cell(10, 5));
    nav.worldFor('gamma').findPath(cell(2, 5), cell(10, 5));
    const claims = ['alpha', 'beta', 'gamma'].map((id) => nav.claimedCell(id)!);
    expect(new Set(claims.map((c) => tileKey(c.x, c.z))).size).toBe(3);
    for (const claim of claims) {
      expect(Math.abs(claim.x - 10) + Math.abs(claim.z - 5)).toBeLessThanOrEqual(
        CLAIM_SEARCH_RADIUS,
      );
    }
...
describe('the navigator fills the GuardWorld port (FR-003, FR-005)', () => {
  ...
  it('drives a real guard end to end on the real level grid', () => {
    ...
    const travelled = Math.hypot(guard.x - start.x, guard.z - start.z);
    expect(travelled).toBeGreaterThan(1);
    expect(isTileBlocking(LEVEL_GRID, Math.floor(guard.x), Math.floor(guard.z), new Set())).toBe(
      false,
    );
    expect(nav.report().searches).toBeGreaterThan(0);
    expect(nav.report().nodesExpanded).toBeLessThanOrEqual(
      nav.report().searches * MAX_NODE_EXPANSIONS,
    );
  });
