...
- 2026-08-31 | 005-materials | 002's five wall type IDs collapse onto three materials (`1`->stone, `2`/`4`->brick, `3`/`5`->steel): US3-S3 and US3-S4 need doors, secrets, floor and ceiling to carry materials no wall type uses, and five cannot cover nine surface classes disjointly.
- 2026-08-31 | 005-materials | A merged mesh's surface is named from its own world triangles against the grid, not from a label: labelling would mean editing 002's and 004's files, and a triangle on no structural plane is how a prop is told from a wall.
- 2026-08-31 | 005-materials | `src/main.ts` reads `performance.now()` in the frame callback instead of the rAF timestamp: a rAF timestamp predates this loop's start after a long blocking setup, so the first delta came out negative and the harness saw 4.8 fps on a page running far faster.
- 2026-08-31 | 005-materials | The one-time driver cost of skinning the level — translating five physical shaders and uploading fifteen 512² maps with their mip pyramids — is forced to completion at load, in the materials system's first `resize`, by a one-pixel `readPixels`. `render()` alone only queues the work and neither `compile()` nor `gl.finish()` waits for it, so on a software renderer it landed as a single ~150ms frame against a ~26ms steady one; since the harness reads `__diag.fps` off a trailing window as soon as the first frame flips `ready`, that frame was most of the average and reported a loop below a floor it clears (US3-S11).
- 2026-08-31 | 005-materials | Declared anisotropy is 4, not 8: it is a per-fragment tap count and the one material setting a software rasterizer pays for, and with three maps sampled per pixel 8->4 cut the measured software frame from 27ms to 22.5ms while staying genuine anisotropic filtering. On real hardware the two are indistinguishable in cost.
- 2026-08-31 | 005-materials | US3 adopts `tools/smoke-check-runner.mjs` rather than keeping the discovery loop T030 put inline in `tools/smoke.mjs`: 006-enemies US3 had already lifted the same seam into that module, and two runners meant two module contracts — a check exporting `check` was silently not run by the one that wants `default`.
