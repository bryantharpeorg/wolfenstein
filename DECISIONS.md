...
- 2026-08-31 | 007-combat-hud | A shot's reported distance is where the ray meets the guard's hit cylinder, not the distance to the guard's centre, because that is the number the wall comparison has to be made against — a body whose near face is closer than the wall absorbs the shot even when its centre is not.
- 2026-08-31 | 007-combat-hud | Fire credit past one interval is dropped rather than carried, so a minute spent standing still cannot be cashed in as a burst on the first press; the same clamp is what makes `MAX_SHOTS_PER_STEP` a real cap rather than a deferral.
- 2026-08-31 | 007-combat-hud | `MAX_SHOTS_PER_STEP` bounds a single frame above what any real delta needs (a quarter-second frame asks for five of the eight), so the cap never touches the rate FR-004 measures and only catches a restored tab.
- 2026-08-31 | 007-combat-hud | An ammo refusal leaves the interval timer standing rather than consuming it, so a magazine refilled on the next frame fires immediately; consuming it would make an empty weapon quietly slower than a full one.
- 2026-08-31 | 007-combat-hud | Spread is bounded by construction — the declared maximum times the square root of a uniform — rather than by a rejection loop, so no draw can return a wider vector and the bound needs no retry to hold.
- 2026-08-31 | 007-combat-hud | The spread bound is measured through the cross product (`atan2(|a×b|, a·b)`) rather than `acos` of the dot: at the angles a pistol cone actually produces, `acos` near one loses most of its significant digits and would report violations that are not there.
- 2026-08-31 | 007-combat-hud | FR-002's "not as literals at call sites" is enforced by scanning every file that imports the weapon table, with values below ten and without a decimal point exempted — an ammo cost of one is indistinguishable from an array index, and an exemption stated in the test is honest where a silent one would not be.
- 2026-08-31 | 007-combat-hud | US1 counts a kill from a guard newly in `death` rather than from its own damage bookkeeping, because nothing but the player's fire damages a guard at this milestone and a set of counted ids is what keeps one death from scoring twice.
