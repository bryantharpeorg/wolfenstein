...
- 2026-08-30 | 004-interaction | US1-S5 and US1-S6 are reconciled by making `refusing-closing` the closing-specific member of a declared moving-refusal class (`isMovingRefusal` in `outcomes.ts`) rather than a rival answer to `blocked-moving`. Read literally the two scenarios conflict — S5 asks for `blocked-moving` from a closing door and S6 asks for `refusing-closing` from the same door — and no implementation satisfies both strings. What they agree on is the behaviour: refuse, leave state and progress unchanged, never reverse or re-trigger, and that holds for both moving states. So the class carries S5's guarantee for `opening` and `closing` alike, and the name carries S6's and FR-003's requirement that a closing door say why it will not re-open.
