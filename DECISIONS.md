...
- 2026-08-31 | 006-enemies | `view-angle.ts` measures a bearing in the codebase's own yaw convention (`atan2(-dx, -dz)`, the one `step.ts`'s `yawToward` and `camera.rotation.y` already use) rather than a clockwise compass: a bearing that had to be reflected before it could be subtracted from a guard's facing would show a guard's back at the moment it turned to face the player, and no test of column *distinctness* would ever catch it.
- 2026-08-31 | 006-enemies | The billboard turns about Y alone, so its normal points at the camera in the horizontal plane and the guard stays upright — a full `lookAt` would lay the sprite back as the player looked down, which is the one way a billboard can look worse than the axis-aligned card US4-S1 forbids.
- 2026-08-31 | 006-enemies | The guard material is an alpha *cutout* (`alphaTest`, `transparent: false`), not a blend: three.js draws a double-sided transparent material in two passes, which would cost two draw calls per visible guard against US4-S7's budget of one, and the procedurally drawn sheet has no partial alpha to blend.
- 2026-08-31 | 006-enemies | `enemySprites` reaches `__diag` by module augmentation from `systems/enemy-billboards/register.ts`, as an optional field: US3 has already made this spec's one edit to `src/diag/diag.ts`, and an optional field needs no change to `createDiagnostics` either.
- 2026-08-31 | 006-enemies | The harness's orbit overrides the *camera*, re-applied at order 65, and never moves the player: `step.ts` snaps an alerted guard's facing straight at the player each tick, so orbiting by teleporting the player would circle a guard pivoting to keep its front to the viewer — the one arrangement in which eight bearings cannot yield eight columns.
- 2026-08-31 | 006-enemies | A culled guard still has its bearing computed and published: `viewAngle` is the guard's relation to the viewer and stays true of a guard the camera has turned away from, while `visible` is what the frame drew — one field per question rather than one field answering two.
- 2026-08-31 | 006-enemies | The update that first sees `death` renders the first death frame rather than adding that frame's delta to the clock: a slow frame must not step over the start of a four-frame animation, and the clock only ever grows afterwards, which is what makes "advances and then holds" true at any frame rate.
