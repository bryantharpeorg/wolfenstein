...
- 2026-08-31 | 006-enemies | `stepGuard` never mutates the guard it is given and returns a fresh record; the one thing it advances in place is the `Rng` handle in its context, and every tick reports `randomConsumed` — FR-002 names the PRNG state as an input, and a per-tick flag is what lets US1-S9 assert a different seed diverges *only* where randomness was drawn.
- 2026-08-31 | 006-enemies | The idle patrol draws both its heading and its jitter on every idle tick, using the heading only when one is due — the generator's position then depends solely on how many idle ticks have passed, never on a branch, which is what makes the trace comparison exact rather than approximately stable.
- 2026-08-31 | 006-enemies | `StepContext.doorStates` is 004's open-tile key set (`ReadonlySet<string>` of `tileKey(x, z)`) rather than a second door-state shape — `openTiles()` already produces it and `isTileBlocking` already consumes it, so US2's nav binding is a pass-through.
- 2026-08-31 | 006-enemies | A chasing guard moves only along a path the injected world returned, never straight at the player — so a stale path discarded mid-chase leaves the guard standing for at most one throttle interval rather than beelining to a destination no route reaches.
- 2026-08-31 | 006-enemies | The guard record splits out of `step.ts` into `src/enemy/guard.ts`, one module more than plan.md's structure sketch names — `step.ts` crossed the 400-line ceiling when the chase and attack behaviour landed, and Article IV makes the split part of the task rather than a later tidy. `step.ts` still declares the `GuardWorld` port and re-exports the record, so no caller's import changes.
