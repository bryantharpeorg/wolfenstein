...
- 2026-08-31 | 005-materials | Shadow-mapped point lights work on both backends the harness exercises, so FR-014's omission line is not appended and `__diag.materials.fallbacks` is empty on the shipped build; `?noshadows=1` drives the degraded path instead, because a degradation with no trigger is one nothing ever proves works.
- 2026-08-31 | 005-materials | Four lamps are planned and `SHADOW_CASTING_LIGHTS = 2` of them carry a shadow map, the two nearest 002's spawn and exit anchors: FR-012 asks for at least two, and on the software renderer the smoke gate uses four cube maps cost a third of the frame rate two cost.
- 2026-08-31 | 005-materials | `LIGHT_DECAY` is 1, not the physical 2 — a corridor here is twenty tiles long, and at inverse square the lamp that reaches the far end blows out the tile it hangs over.
- 2026-08-31 | 005-materials | `FOG_FAR` is 90 against a longest sight-line of 61 units, measured from 002's grid by `longestSightLine()` rather than asserted against a comment, so a level edit that lengthens a corridor fails `npm run test` instead of fogging the exit away.
- 2026-08-31 | 005-materials | The shadow probe applies the sRGB transfer function to the bytes it reads back: three writes a non-XR render target in the linear working space whatever its texture's colour space says, and "not pure black" is a claim about what a player sees.
- 2026-08-31 | 005-materials | `MaterialFallback` gains a `'shadow'` kind and a `'lighting'` name — FR-014 puts the rig's one degradation in `__diag.materials.fallbacks`, and recording it as a binding failure of some innocent material would be a lie on the page; the widening renames and repurposes nothing.
- 2026-08-31 | 005-materials | The lighting system removes the lights already in the scene before adding its own rather than editing 002's `src/systems/level/register.ts`, or the ambient the page renders would not be the one FR-013 declares.
