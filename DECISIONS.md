...
- 2026-08-31 | 007-combat-hud | FR-011's cross-spec reset goes through one additive accessor exported by each owning system, because doors, secrets, keys and guards keep live state in module locals with no reader; the reset policy still lives only in `reset-adapters.ts`.
- 2026-08-31 | 007-combat-hud | The enemies system rebuilds its world on restart rather than rewinding it, because the tick accumulator and queued per-guard damage are closure-private and a rewind would land a pre-restart shot on a post-restart guard.
- 2026-08-31 | 007-combat-hud | A collected key's mesh is hidden rather than removed, so restart can put it back and FR-018's draw-call budget is unchanged.
- 2026-08-31 | 007-combat-hud | The restart command is serviced once per frame rather than run on arrival, so two presses in quick succession are one reset.
- 2026-08-31 | 007-combat-hud | `snapshotRunState` is a flat dotted field map carrying run state and not presentation, so SC-002's comparison is a key walk, the exempt set is a list `008` extends in one place, and `bobOffset` — whose phase a spawn teleport perturbs for one frame — is left out.
- 2026-08-31 | 007-combat-hud | Kills are scored before damage is applied within the vitals frame, so a guard killed on the tick its shot kills the player still scores and the death still counts once.
- 2026-08-31 | 007-combat-hud | US2-S7's per-guard `idle` claim is asserted in the equal-simulation-age snapshot pair rather than by re-reading guard state some frames after a restart, because guard 0 sees the spawn tile and goes `alert` on its second tick in a freshly built world.
