...
- 2026-08-30 | 004-interaction | A secret travels at the door's own rate — `SECRET_TILE_MS = DOOR_TRAVEL_MS / DOOR_TRAVEL_TILES`, so its two tiles take twice a door's one — rather than declaring a second duration in `params.ts`, which US1 owns and US3 must not reopen.
- 2026-08-30 | 004-interaction | `found` latches when travel actually begins, so a secret whose first tile is solid is refused with `blocked-geometry` and never counted — `secretsFound` then names openings the player can walk through rather than presses they made.
- 2026-08-30 | 004-interaction | `secretRemainingTiles` is added to `InteractionDiagnostics` by module augmentation from `secret-field.ts` — US3-S6 wants the shortfall reported in diagnostics, and FR-017's object is declared in US1's file, which the plan forbids reopening.
- 2026-08-30 | 004-interaction | The secrets system installs its own `keydown` listener resolving through the one `bindings.ts` table, and records a reason only when a secret was in reach — FR-005 binds one command *path*, not one listener, and a press at a door still reports the door's own answer.
- 2026-08-30 | 004-interaction | The tile a pushed wall comes to rest on is not re-blocked for collision — the passable-tile registry only adds tiles, and inverting it would mean editing `open-state.ts` and 003's collider, which no US3 task owns. Every shipped secret retreats into open room, so nothing is reachable behind it that was not already.
