...
- 2026-08-31 | 006-enemies | `attack.ts` takes its blocking verdict from `los.ts` and walks the same DDA only to report *where* the ray died, which a boolean cannot carry — a battery of rays through both keeps the two answers identical rather than trusting them to stay so.
...
- 2026-08-31 | 006-enemies | The falloff curve is piecewise-linear between breakpoints and clamped outside them, so `damageAtDistance` is total; its declared range contains `ATTACK_RANGE_CELLS`, so no real shot is evaluated on the clamped tail.
- 2026-08-31 | 006-enemies | "No shot emitted" is a *state* question: `resolveShot` answers null for every state but `attack`, because a wall between guard and player is `blocked` and US3-S5's silence is the guard never having reached `attack`.
- 2026-08-31 | 006-enemies | Sight and the bullet are one walk: `los.ts` leaves the cell that stopped it in `lastBlock` and `attack.ts` reads it, rather than a second DDA kept in step by a test battery. The scratch is module-scope so sight still allocates nothing per call (US2-S8).
- 2026-08-31 | 006-enemies | A spawn marker on a wall is recorded as a named error with the guard still instantiated — dropping it would make the live count disagree with the marker count, and throwing would say "something crashed" instead of naming coordinates. It goes to `__diag.enemySpawnErrors`, not 001's `errors`, and `tools/smoke-checks/enemies.mjs` fails the gate on it.
- 2026-08-31 | 006-enemies | Guard AI runs at a fixed `GUARD_TICK_MS` with at most `MAX_TICKS_PER_FRAME` of catch-up and the backlog dropped: a stalled tab must not come back and run hundreds of ticks of pathing in one frame.
- 2026-08-31 | 006-enemies | `pathable` lives on the world's record, not on the `Guard`: an unreachable answer outlives the tick that asked, so the guard returns to `idle` at once and stays reported unpathable until a request returns a route. `world.ts` re-states `step.ts`'s idle entry rather than opening US1's file to export `onEnter`.
