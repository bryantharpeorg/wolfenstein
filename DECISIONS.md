...
- 2026-08-31 | 007-combat-hud | The HUD is one canvas-2D texture on one screen-space quad parented to the camera, and the HUD system adds the camera to the scene itself — `main.ts` never added it, and adding it from a system rather than there is exactly what the registry exists for.
- 2026-08-31 | 007-combat-hud | The muzzle-flash clock is elapsed-since-the-shot plus a lit flag, not a decrementing remainder — US4-S7 asks for *exactly* zero, and a remainder stepped by a hundred floating-point frame deltas can settle a whisker above it and leave the flash alight for the rest of the run.
- 2026-08-31 | 007-combat-hud | `drawHud` skips the redraw when the readout's signature is unchanged: a change detector derived from the same values it draws, not a second copy of them, so a still HUD costs no texture upload and cannot disagree with the state it was computed from.
- 2026-08-31 | 007-combat-hud | The lowest portrait band is one point, so a reading below it shows the death portrait — there is no living face for a player with less than a point left, and the alternative is a band whose whole width is a rounding error.
- 2026-08-31 | 007-combat-hud | The HUD publishes what it composited through `window.__hud.drawn()`, because US4-S2 and US4-S3 are claims about the *displayed* values and a canvas texture cannot be read back as text; it is by copy, so a live handle cannot silently agree with every later change.
- 2026-08-31 | 007-combat-hud | `viewmodel.ts` cites its requirement by scenario rather than by FR number — `weapons.test.ts` scans every importer of the weapon table for a line restating one of its values, and `FR-017` reads as `17`, which is the SMG's damage; the requirement is stated in full in `flash.ts`, which imports no table.
- 2026-08-31 | 007-combat-hud | US4's full-loop driving lives in `tools/smoke-loop.mjs` while `tools/smoke.mjs` keeps the assertions and the step names — following 006's precedent, because that harness is already past the 400-line ceiling and this story is not the one that rewrites it.
- 2026-08-31 | 007-combat-hud | The full-loop pass restarts once mid-run before walking the portrait bands, so the ladder is descended from full health — the guards fire from load, and a walk starting from whatever they left would leave the healthiest portraits untested on the built page.
