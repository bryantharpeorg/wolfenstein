...
- 2026-08-31 | 006-enemies | `__diag.enemies`, `enemiesAlive` and `enemyBillboards` reach the diagnostics object by TypeScript module augmentation in `src/enemy/enemy-diag.ts`, the idiom 003 and 004 already use, rather than by editing `src/diag/diag.ts` — FR-011's "extended additively" then holds by construction, and US4 never opens a file another story owns.
- 2026-08-31 | 006-enemies | The billboard system draws whatever guard records are already published on `__diag.enemies` and stands one stationary guard on each level spawn marker only when nothing has published any — US4 renders before US3's `world.ts` exists without growing a second world beside it, and stops falling back the moment US3 lands.
- 2026-08-31 | 006-enemies | The orbit is driven by `window.__enemyView` on the enemy-billboards system — a camera seam of its own, the counterpart of 003's `window.__playerDrive` — rather than by reaching into 003's player state, because 003 landed no programmatic camera hook (plan.md, Complexity Tracking).
- 2026-08-31 | 006-enemies | The `tools/smoke-checks/*.mjs` discovery loop landed in US4 rather than US3, because US3 had not landed when US4 needed it; T027's own instruction is to reuse such a loop and add no line. It lives in `tools/smoke-check-runner.mjs` so the 800-line harness grows by an import instead of a function, and so `smoke-checks/` holds checks and nothing else.
- 2026-08-31 | 006-enemies | A guard's frame is selected by rewriting the four UVs of its own quad against one shared material, not by cloning the texture per guard — a frame change costs four floats, and ten guards still cost the single upload US4-S7 asks for.
- 2026-08-31 | 006-enemies | Billboards rotate about Y only, never about the camera's full orientation: a guard stands on the floor, and matching the camera's pitch would tip the whole figure off its feet as the player looks down.
