...
- 2026-08-31 | 006-enemies | `__diag.enemies`, `enemiesAlive` and `enemyBillboards` are added by module augmentation in `src/enemy/enemy-diag.ts` — 003's and 004's idiom — not by editing `src/diag/diag.ts`, so FR-011's "additively" holds by construction.
- 2026-08-31 | 006-enemies | The billboard system draws whatever records `__diag.enemies` already carries, and stands a guard on each spawn marker only when nothing has published any: US4 renders before US3's `world.ts` exists without growing a second world.
- 2026-08-31 | 006-enemies | The orbit is driven by `window.__enemyView`, the counterpart of 003's `window.__playerDrive`, because 003 landed no programmatic camera hook (plan.md, Complexity Tracking).
- 2026-08-31 | 006-enemies | The `tools/smoke-checks/*.mjs` loop landed in US4, not US3, which had not landed when US4 needed it; T027 says to reuse such a loop. It sits in its own file so the harness grows by an import.
- 2026-08-31 | 006-enemies | A frame is selected by rewriting one quad's four UVs against a shared material, so ten guards still cost the single texture upload US4-S7 asks for.
- 2026-08-31 | 006-enemies | Billboards rotate about Y only: a guard stands on the floor, and matching the camera's pitch would tip the figure off its feet.
- 2026-08-31 | 006-enemies | US4's own `tools/smoke-check-runner.mjs` is dropped for US3's — reverses the 2026-08-31 entry claiming the loop landed in US4, because US3 landed on main first and T027 says to reuse an existing loop rather than keep a second one; US4's check module moves to the default-export contract that runner declares.
- 2026-08-31 | 006-enemies | `enemies` and `enemiesAlive` come from US3's `src/diag/diag.ts` and only `enemyBillboards` stays a module augmentation — reverses the 2026-08-31 entry declaring all three by augmentation, because re-declaring a landed field with a different type is a compile error rather than an additive extension.
- 2026-08-31 | 006-enemies | The billboards read US3's live records through `getEnemyWorld()` and write each bearing back with `setViewAngle` — reverses the 2026-08-31 entry about standing a guard on each spawn marker, which existed only so US4 could render before its sibling landed.
- 2026-08-31 | 006-enemies | The smoke gate kills a guard through `__enemyView.kill()` onto US3's `damageGuardById`, not by assigning `death` to the published record: the world rewrites that record every tick, so only the shipped damage path actually reaches US4-S5 and US4-S6.
